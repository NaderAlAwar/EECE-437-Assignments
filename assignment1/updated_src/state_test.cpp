...
int main(){
	...
	State test(temp);
	cout << test.get_value();
	test.set_value("state 1 updated");
	cout << test.get_value() << endl;
	vector<int> input{ 1, 5 };
	Condition lessThan10(l10);
	lessThan10.update_variables(input);
	cout << "Condition less than 10 is " << (lessThan10.is_true()?"true":"false")<< endl;
	
	cout << "Before applying action, input[0] is " << input[0] << endl;
	Action a5(iT);
	a5.update_variables(input);
	a5.print_variables();
	a5.execute_action();
	a5.print_variables();
	...
	test1.print_members();
	cout << "attempting transition (1 if successful 0 if not) " << test1.attempt_transition() << endl;
	test1.disable_port();
	test1.print_members();
	cout << "attempting transition (1 if successful 0 if not) " << test1.attempt_transition() << endl;
	...
	Transitions group = test1 + test2 + test3;
	State yellow("yellow");
